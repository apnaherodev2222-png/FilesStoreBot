...
def reply_my_files(message):
    ...
    if not keys:
        bot.reply_to(message,
                     ...
                     "║   📂  <b>MY FILES</b>          ║\n"
                     "╚═══════════════════════╝\n\n"
                     "<i>No files found yet.</i>\n"
                     "<i>Tap 📤 Upload File to add your first one!</i>",
                     reply_markup=get_admin_reply_keyboard())
        return
    ...
    lines = [
        ...
        "║   📂  <b>MY FILES</b>          ║",
        "╚═══════════════════════╝",
        f"<i>📦 {len(keys)} file key(s) stored</i>",
        "━━━━━━━━━━━━━━━━━━━━━━━\n",
    ]
    for i, row in enumerate(keys, 1):
        exp = "♾️" if not row["expires"] else datetime.fromtimestamp(row["expires"]).strftime("%d/%m %H:%M")
